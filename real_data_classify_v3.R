...
#   summarize(avg_level=mean(level,na.rm=TRUE)) %>% filter(!is.na(avg_level)) %>% spread(key=BSSID,value=avg_level)
for_dist <- select(wifi_data,BSSID,level,timestamp,speech) %>%  spread(key=BSSID,value=level)
to_dist <- lapply(select(for_dist,-timestamp),function(x) {
  x[is.na(x)] <- -999
  x
}) %>% as_data_frame()
...
gps_data <- select(join_data,timestamp,latitude,longitude)
combined <- left_join(to_dist,gps_data,by='timestamp') %>% distinct()
