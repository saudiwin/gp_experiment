...
model {
  ...
  latitude ~ normal(alpha_lat + X*theta_lat,sigma_lat);
  longitude ~ normal(alpha_lon + X*theta_lon,sigma_lon)
}
...
generated quantities {
  ...
  for(n in 1:N) {
    lon_predict[n] = normal_rng(alpha_lat + X[n,]*theta_lat,sigma_lat);
    lat_predict[n] = normal_rng(alpha_lon + X[n,]*theta_lon,sigma_lon);
  }  
}